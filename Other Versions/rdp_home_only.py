#!/usr/bin/env python3
# rdp_home_only.py — Exact same GUI as rdp_ssd_share.py, but ONLY shares folders inside your home
...
class RDPHomeOnly:
    def __init__(self, root):
        self.root = root
        self.root.title("RDP – Share Folder from Home")
        self.root.geometry("680x720")
        self.root.minsize(620, 650)
        self.root.configure(padx=20, pady=20)

        self.config_file = os.path.expanduser("~/.rdp_home_only_config.json")
        self.process = None
    ...
    def create_widgets(self):
        ...
        self.server = ttk.Entry(self.root, width=60, font=("Consolas", 11))
        ...
        # Folder picker — ONLY inside home
        share = ttk.LabelFrame(self.root, text=" Choose Folder Inside Your Home ")
        ...
        self.name_entry.insert(0, "HomeFolder")
        ...
        self.status = ttk.Label(self.root, text="Ready – Pick a folder inside your home", foreground="sea green", font=("", 11, "bold"))
        self.status.pack(pady=20)

    def browse_path(self):
        path = filedialog.askdirectory(initialdir=os.path.expanduser("~/"), title="Select folder inside your home")
        if path and path.startswith(os.path.expanduser("~/")):
            self.path_entry.delete(0, tk.END)
            self.path_entry.insert(0, path)
        elif path:
            messagebox.showwarning("Blocked", "You can only share folders inside your home directory")
    ...
    def connect(self):
        ...
        name   = self.name_entry.get().strip() or "Folder"
        ...
        self.status.config(text=f"Connected – '{os.path.basename(path)}' shared as '{name}'", foreground="sea green")
    ...
    def save_config(self):
        ...
        try:
            ...
            if HAS_KEYRING and self.passwd.get():
                keyring.set_password("rdp_home", f"{self.user.get()}@{self.server.get()}", self.passwd.get())
    ...
    def load_config(self):
        if os.path.exists(self.config_file):
            try:
                ...
                self.path_entry.delete(0, tk.END)
                self.path_entry.insert(0, c.get("path", os.path.expanduser("~/Documents")))
                self.name_entry.delete(0, tk.END)
                self.name_entry.insert(0, c.get("name", "HomeFolder"))
                self.fullscreen.set(c.get("fullscreen", False))
                self.multimon.set(c.get("multimon", True))
                self.sound.set(c.get("sound", True))
                if HAS_KEYRING:
                    stored = keyring.get_password("rdp_home", f"{c.get('user','')}@{c.get('server','')}")
