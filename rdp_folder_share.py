#!/usr/bin/env python3
# rdp_folder_share.py — Full GUI with FREE folder selection (2025)
...
class RDPFolderShare:
    def __init__(self, root):
        self.root = root
        self.root.title("RDP – Share Any Folder")
        self.root.geometry("680x720")
        self.root.minsize(620, 650)
        self.root.configure(padx=20, pady=20)

        self.config_file = os.path.expanduser("~/.rdp_folder_share.json")
        self.process = None
    ...
    def create_widgets(self):
        ttk.Label(self.root, text="RDP Folder Sharer", font=("Segoe UI", 22, "bold")).pack(pady=(0, 30))
        ...
        # Folder picker — FULL freedom
        share = ttk.LabelFrame(self.root, text=" Choose Any Folder to Share ")
        ...
        self.name_entry.insert(0, "Shared")
        ...
        self.status = ttk.Label(self.root, text="Ready – Pick any folder and connect", foreground="sea green", font=("", 11, "bold"))
        self.status.pack(pady=20)

    def browse_path(self):
        path = filedialog.askdirectory(initialdir=os.path.expanduser("~/"), title="Select any folder to share")
        if path:
    ...
    def save_config(self):
        ...
        try:
            ...
            if HAS_KEYRING and self.passwd.get():
                keyring.set_password("rdp_folder", f"{self.user.get()}@{self.server.get()}", self.passwd.get())
